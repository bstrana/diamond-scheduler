...
const scoreEditFromRecord = (r: any): ScoreEdit => ({
  ...
  pitcher:     r.pitcher    || undefined,
  pitchCount:  typeof r.pitch_count === 'number' ? r.pitch_count : undefined,
  batter:      r.batter     || undefined,
  batting:     r.batting    || undefined,
  showRecap:   r.show_recap !== false,
...
export const saveScoreEdit = async (edit: Omit<ScoreEdit, 'id' | 'updated'>): Promise<boolean> => {
  ...
  try {
    ...
    const payload = {
      ...
      pitcher:      edit.pitcher    ?? '',
      pitch_count:  edit.pitchCount ?? null,
      batter:       edit.batter     ?? '',
      batting:      edit.batting    ?? '',
      show_recap:   edit.showRecap ?? true,
