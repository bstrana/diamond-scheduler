...
import { ChevronLeft, ChevronRight, MapPin, Calendar as CalIcon, Clock } from 'lucide-react';
...
const GameBar: React.FC<GameBarProps> = ({
  ...
  // Filter games - only scheduled games from today onwards
  const filteredGames = useMemo(() => {
    return games.filter(g => {
      // Only scheduled games from today onwards
      if (g.date < todayStr || g.status !== 'scheduled') {
        return false;
      }
  ...
  }, [games, selectedTeamId, selectedLeagueId, selectedCategory, leagues, todayStr]);
  ...
  return (
    <div 
      className="h-full flex rounded-xl shadow-sm"
      style={{
    ...
    >
      {/* Left Sidebar with Filters - Stacked Vertically */}
      {!hideFilters && (
        <div 
          className="w-48 border-r p-4 flex flex-col space-y-3 shrink-0"
          style={{
              ...
              filteredGames.map(game => {
                ...
                return (
                  <div
                    ...
                    style={{
                      backgroundColor: 'var(--embed-card-bg, #ffffff)',
                      border: 'var(--embed-border-width, 1px) solid var(--embed-card-border, #e2e8f0)',
                      borderRadius: 'var(--embed-card-radius, 0.5rem)',
                      boxShadow: 'var(--embed-card-shadow, 0 1px 3px 0 rgba(0, 0, 0, 0.1))',
                      padding: 'var(--embed-padding, 1rem)'
                    }}
                    onMouseEnter={(e) => {
                      e.currentTarget.style.borderColor = 'var(--embed-primary, #6366f1)';
                      e.currentTarget.style.boxShadow = 'var(--embed-card-shadow, 0 10px 15px -3px rgba(0, 0, 0, 0.1))';
                    }}
                    onMouseLeave={(e) => {
                      e.currentTarget.style.borderColor = 'var(--embed-card-border, #e2e8f0)';
                      e.currentTarget.style.boxShadow = 'var(--embed-card-shadow, 0 1px 3px 0 rgba(0, 0, 0, 0.1))';
                    }}
                  >
                    {/* Date Header with Time and Location */}
                    <div 
                        ...
                        <div className="flex flex-col items-end space-y-1">
                          ...
                          <div className="flex items-center space-x-1 text-xs">
                            ...
                            <span 
                              ...
                              style={{ 
                                color: isToday ? 'rgba(255, 255, 255, 0.9)' : 'var(--embed-text, #64748b)'
                              }}
                            >
                              {game.location}
                            </span>
