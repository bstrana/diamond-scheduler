  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (editingId) {
      ...
      if (maxTeams && teams.length >= maxTeams) {
        alert(t('teams.teamLimitReached', { limit: maxTeams }));
        return;
      }
      // Add new team
      onAddTeam({
  ...
  return (
    <div className="max-w-5xl mx-auto">
      <div className="flex justify-between items-center mb-6">
        <h2 className="text-2xl font-bold text-slate-800">{t('teams.leagueTeams')}</h2>
        {!isFormVisible && (
          <button
      ...
      {isFormVisible && (
        <div ref={formRef} className="bg-white p-6 rounded-xl shadow-md border border-indigo-100 mb-8 animate-in fade-in slide-in-from-top-4">
          <div className="flex justify-between items-center mb-4">
              <h3 className="font-semibold text-slate-700">{editingId ? t('teams.editTeam') : t('teams.newTeam')}</h3>
          ...
          <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-6 gap-4 items-end">
            <div className="lg:col-span-1">
              <label className="block text-xs font-semibold text-slate-500 uppercase mb-1">{t('teams.city')}</label>
              <input required placeholder={t('teams.cityPlaceholder')} className="w-full border p-2 rounded" value={formTeam.city || ''} onChange={e => setFormTeam({...formTeam, city: e.target.value})} />
            </div>
            <div className="lg:col-span-1">
              <label className="block text-xs font-semibold text-slate-500 uppercase mb-1">{t('teams.name')}</label>
              <input required placeholder={t('teams.namePlaceholder')} className="w-full border p-2 rounded" value={formTeam.name || ''} onChange={e => setFormTeam({...formTeam, name: e.target.value})} />
            </div>
            <div className="lg:col-span-1">
              <label className="block text-xs font-semibold text-slate-500 uppercase mb-1">{t('teams.country')}</label>
              <select
            ...
            <div className="lg:col-span-1">
              <label className="block text-xs font-semibold text-slate-500 uppercase mb-1">{t('teams.abbr')}</label>
              <input required maxLength={3} placeholder={t('teams.abbrPlaceholder')} className="w-full border p-2 rounded uppercase" value={formTeam.abbreviation || ''} onChange={e => setFormTeam({...formTeam, abbreviation: e.target.value.toUpperCase()})} />
            </div>
            <div className="lg:col-span-1">
              <label className="block text-xs font-semibold text-slate-500 uppercase mb-1">{t('teams.primaryColor')}</label>
            ...
            <div className="lg:col-span-1">
              <label className="block text-xs font-semibold text-slate-500 uppercase mb-1">{t('teams.secondaryColor')}</label>
            ...
            <div className="lg:col-span-1">
              <label className="block text-xs font-semibold text-slate-500 uppercase mb-1">{t('teams.logoUrl')}</label>
              <input
                type="url"
              ...
              />
              <p className="text-xs text-slate-500 mt-1">{t('teams.logoUrlHelp')}</p>
              {formTeam.logoUrl && (
            ...
            <div className="lg:col-span-6">
              <label className="block text-xs font-semibold text-slate-500 uppercase mb-1">{t('teams.roster')}</label>
              <textarea
                rows={4}
                className="w-full border p-2 rounded text-sm"
                placeholder={t('teams.rosterPlaceholder')}
                value={rosterText}
                onChange={(e) => setRosterText(e.target.value)}
              />
              <p className="text-xs text-slate-500 mt-1">{t('teams.rosterHelp')}</p>
            </div>
            <div className="flex space-x-2 lg:col-span-1">
              <button type="button" onClick={handleCancel} className="flex-1 px-4 py-2 border rounded hover:bg-slate-50 text-sm">{t('common.cancel')}</button>
              <button type="submit" className="flex-1 px-4 py-2 bg-indigo-600 text-white rounded hover:bg-indigo-700 text-sm font-medium">
                  {editingId ? t('teams.update') : t('teams.save')}
              </button>
            </div>
          </form>
        </div>
      ...
      {teams.length === 0 ? (
        <div className="text-center py-20 bg-slate-50 rounded-xl border-2 border-dashed border-slate-200">
          ...
          <h3 className="text-lg font-medium text-slate-900">{t('teams.noTeamsYet')}</h3>
          <p className="text-slate-500">{t('teams.noTeamsHint')}</p>
        </div>
      ) : (
